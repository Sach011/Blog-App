...
export const createBlog = async (req, res) => {
    try {
        ...
        return res.status(400).json({message: "Blog image is missing."});
    }
    const { blogImage } = req.files;
    const allowedFormats = ["image/jpg", "image/jpeg", "image/png", "image/gif"]
    if(!allowedFormats.includes(blogImage.mimetype)){
        return res.status(400).json({ message: "Invalid file format." });
        console.log(blogImage.mimetype)
    }
    const { title, category, about,jwt } = req.body;
    if(!title || !category || !about){
        return res.status(400).json({ message: "title, category and about is missing" });
        }
    ...
    if (!cloudinaryResponse2 || cloudinaryResponse2.error) {
        console.log(cloudinaryResponse2.error);
    }
    const blogData = {
        title,
        about,
